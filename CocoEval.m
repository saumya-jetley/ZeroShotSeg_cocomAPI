       ...
       if (t==4) %sj shape amendment
           ...
           detpath = params.detpath;
            ...
            for i=1:size(g,1)
                ...
                for P_ind=1:size(P,1)
                    g_pos=P{P_ind};
                    G_im = G_im | poly2mask(g_pos(1:2:end),g_pos(2:2:end),480,640);
                end
                if size(d,1)==0 
                    break;
                else
                    for j=1:size(d,1)
                        D_im = imread(strcat(detpath,d{j}));
                        D_im =imresize(D_im,[480,640],'nearest');
                        dim = D_im>0;
                        inter = sum(sum(G_im.*dim));
                        union = sum(sum(G_im|dim));
                        ious(j,i)=double(inter/union);
                    end
                end
            end
        end
