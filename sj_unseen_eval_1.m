...
%% SJ-version v.1.00 
% - To store image gt and predictions in COCO Json format
% - All the categories get single label - 81 (unseen cat label)

%% Find the unseen category details in sj_unseen_imagesonly mat file
clear;
close all;
clc;
pred_path = '/media/sjvision/DATASETDISK/shape_detection/datasets/coco/unseen_ims_pred_0.05/tmp/';
...
%% Collate the images and annotations in the standard format - modify it for the predictions
...
%     gt_annCatIds = [gt_annCatIds;(ones(size(annIds))*1)'];
    ...
    % Load all the predictions from the image 
...
save('/media/sjvision/DATASETDISK/shape_detection/datasets/coco/annotations/unseen_cat/dt_file_0.05.mat','coco_unseen_dt');
disp('done');